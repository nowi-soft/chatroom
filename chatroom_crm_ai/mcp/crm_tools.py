...
class ChatroomCrmAiTools(models.AbstractModel):
    ...
    def _crm_ai_build_description(self, room, summary):
        parts = [
            ...
            "",
            self.env._(
                "— Automatically generated by the AI assistant from chat '%s'.",
                room.name or room.id,
            ),
        ]
        transcript = self._crm_ai_transcript(room)
        if transcript:
            parts += ["", self.env._("Conversation:"), transcript]
    ...
    def _mcp_create_crm_lead(
        ...
        final_name = (title or "").strip() or self.env._(
            "Lead via chat — %s",
            contact_name or partner.name or room.name,
        )
